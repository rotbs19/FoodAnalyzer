...
public class Glcm {
    ...
    public Glcm(Bitmap image, int pixelSize, int grayLevel) throws IOException {
        this.image = image;
        this.grayLevel = grayLevel;
        // sets the size of pixels to check
        this.pixelSize = pixelSize;
        grayLeveledMatrix = new int[pixelSize][pixelSize];
    }
    ...
    private void createGrayLeveledMatrix() {
        for (int i = 0; i < pixelSize; i++) {
            for (int j = 0; j < pixelSize; j++) {
                ...
                int newR = Color.red(pixel);
                int newG = Color.green(pixel);
                int newB = Color.blue(pixel);
                int newA = Color.alpha(pixel);
    ...
    // retrieves a vector holding all the images features
    public ArrayList<Double> getVector(){
        ArrayList<Double> vector = new ArrayList<>();
        vector.add(contrast);
        vector.add(homogenity);
        vector.add(entropy);
        vector.add(energy);
        vector.add(dissimilarity);
        return vector;
    }
